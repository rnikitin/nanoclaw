...
Messages must start with the trigger pattern (default: `@Ark`):
- `@Ark what's the weather?` → ✅ Triggers Claude
- `@арк help me` → ✅ Triggers (case insensitive)
- `Hey @Ark` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Ark what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Ark what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Ark add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Ark remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Ark list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Ark remember I prefer dark mode` | Add to global memory |
...
```
User: @Ark remind me every Monday at 9am to review the weekly metrics
...
```
User: @Ark at 5pm today, send me a summary of today's emails
...
From any group:
- `@Ark list my scheduled tasks` - View tasks for this group
- `@Ark pause task [id]` - Pause a task
- `@Ark resume task [id]` - Resume a paused task
- `@Ark cancel task [id]` - Delete a task

From main channel:
- `@Ark list all tasks` - View tasks from all groups
- `@Ark schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Ark add group "Name"` in main |
