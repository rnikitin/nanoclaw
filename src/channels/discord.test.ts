...
function createMessage(overrides: {
  ...
  // Thread support — when set, message is treated as arriving inside a thread.
  inThread?: {
    threadId: string;
    threadName?: string;
    parentId?: string;
  ...
  const thread = overrides.inThread;
  const channel: any = thread
    ? {
        id: thread.threadId,
        name: thread.threadName ?? 'thread',
  ...
  const defaultStartThread = vi.fn(async (opts: { name: string }) => ({
    id: 'thread_auto_001',
    name: opts.name,
    parentId: channelId,
  }));
  ...
  return {
    channelId: thread ? thread.threadId : channelId,
    ...
    startThread: overrides.startThread ?? defaultStartThread,
  };
}
...
describe('DiscordChannel', () => {
  ...
  describe('text message handling', () => {
    it('ignores non-mention messages in a registered channel', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
      ...
      // Metadata still recorded, but no message delivery and no thread created.
    ...
    it('creates thread and delivers message on @mention in registered channel', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();

      const startThread = vi.fn(async (o: { name: string }) => ({
        id: 'thread_abc',
        name: o.name,
      ...
      expect(startThread).toHaveBeenCalledWith(
        expect.objectContaining({ name: 'what time is it?' }),
      );
      expect(opts.onMessage).toHaveBeenCalledWith(
        'dc:thread_abc',
        expect.objectContaining({
          chat_jid: 'dc:thread_abc',
          sender: '55512345',
          sender_name: 'Alice',
          content: '@Andy what time is it?',
        }),
      );
      // Parent should now map to the new thread via getParentJid.
      expect(channel.getParentJid('dc:thread_abc')).toBe('dc:1234567890123456');
    });

    it('delivers thread messages without requiring trigger or creating new threads', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
      ...
      const msg = createMessage({
        ...
        inThread: {
          threadId: 'thread_xyz',
          threadName: 'initial topic',
          parentId: '1234567890123456',
        },
      });
      await triggerMessage(msg);

      // Should deliver at the thread JID, no new thread created.
      expect(opts.onMessage).toHaveBeenCalledWith(
        'dc:thread_xyz',
        expect.objectContaining({
          chat_jid: 'dc:thread_xyz',
          content: 'follow-up question',
        }),
      );
      expect(msg.startThread).not.toHaveBeenCalled();
      // Self-heals the thread → parent map from the inbound message.
      expect(channel.getParentJid('dc:thread_xyz')).toBe('dc:1234567890123456');
    });
    ...
    it('records chat metadata for DM channels (no guild)', async () => {
      const opts = createTestOpts({
        registeredGroups: vi.fn(() => ({})),
      });
      const channel = new DiscordChannel('test-token', opts);
    ...
    it('uses guild name + channel name for server metadata', async () => {
      const opts = createTestOpts({
        registeredGroups: vi.fn(() => ({})),
      });
      const channel = new DiscordChannel('test-token', opts);
  ...
  // --- @mention translation (parent-channel path creates thread) ---
  ...
  describe('@mention translation', () => {
    ...
    it('ignores messages in registered channel without mention', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
  ...
  describe('attachments', () => {
    // Attachments attach to thread messages. Tests send them inside a thread
    // (no mention required) — the parent-channel path would require a mention.
    ...
    it('stores image attachment with placeholder', async () => {
      ...
      const msg = createMessage({
        ...
        attachments,
        guildName: 'Server',
        inThread: { threadId: 'thread_att', parentId: '1234567890123456' },
      });
      await triggerMessage(msg);
  ...
  describe('reply context', () => {
    it('includes reply author in content (inside thread)', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
      ...
      const msg = createMessage({
        ...
        guildName: 'Server',
        inThread: { threadId: 'thread_reply', parentId: '1234567890123456' },
      });
      await triggerMessage(msg);
