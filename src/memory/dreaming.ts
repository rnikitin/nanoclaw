...
function savePhaseSignals(groupDir: string, store: PhaseSignalStore): void {
  ...
  try { require('fs').chmodSync(path, 0o666); } catch { /* ignore */ }
}
...
function deduplicateCandidates(
  ...
  for (const entry of entries) {
    const isDup = result.some(e => similarity(e.snippet, entry.snippet) >= threshold);
    if (!isDup) {
      result.push(entry);
    }
  }
...
export function lightSleep(groupDir: string): LightSleepResult {
  ...
  try {
    ...
    logger.warn({ err, group: basename(groupDir) }, 'Knowledge graph rebuild failed (non-fatal)');
  }

  // 5. Write light sleep report
  const reportDir = join(groupDir, '.dreams', 'light-sleep');
  if (!existsSync(reportDir)) mkdirSync(reportDir, { recursive: true, mode: 0o777 });
  const report = {
    phase: 'light',
    timestamp,
...
export function remSleep(groupDir: string): RemSleepResult {
  ...
  try {
    ...
    logger.warn({ err, group: basename(groupDir) }, 'Hierarchy topic building failed (non-fatal)');
  }
...
export function deepSleep(groupDir: string): DeepSleepResult {
  ...
  for (const signal of signalStore.signals) {
    phaseBoosts[signal.contentHash] = (phaseBoosts[signal.contentHash] || 0) + signal.boost;
  }

  // 3. Score and rank (with phase boosts applied)
  const scored = rankCandidates(deduped, DEFAULT_WEIGHTS, DEFAULT_THRESHOLDS, phaseBoosts);
  ...
  if (promoted.length > 0) {
    ...
    try { require('fs').chmodSync(memoryPath, 0o666); } catch { /* ignore */ }
  }
  ...
  try {
    ...
    logger.warn({ err, group: basename(groupDir) }, 'Hierarchy globals failed (non-fatal)');
  }
...
export function setupGroupDreaming(groupDir: string): void {
  ...
  if (!existsSync(recallPath)) {
    writeFileSync(recallPath, JSON.stringify({
      version: 1,
      entries: {},
      lastUpdated: new Date().toISOString(),
    }, null, 2));
    try { require('fs').chmodSync(recallPath, 0o666); } catch { /* ignore */ }
  }
  ...
  if (!existsSync(signalsPath)) {
    ...
    try { require('fs').chmodSync(signalsPath, 0o666); } catch { /* ignore */ }
  }
  ...
  if (existsSync(memoryDir)) {
    ...
    } catch { /* QMD not installed or collection exists */ }
  }
